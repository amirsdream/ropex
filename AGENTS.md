...
| Area | Files |
| --- | --- |
| Spec / reconcile | `spec.ts`, `controller.ts`, `image.ts`, `worktree.ts`, `watch.ts`, `gitrepo.ts` |
...
| Ingress / audit | `webhook.ts`, `ratelimit.ts`, `journal.ts`, `trajectory.ts`, `metrics.ts` |
